...
class ScreenshotOnBoardingTest {
    ...
    @After
    fun tearDown() {
        val settings = ajit.browser.focus.utils.Settings.getInstance(context)
        settings.eventHistory.add(ajit.browser.focus.utils.Settings.Event.ShowMyShotOnBoardingDialog)
    }
    ...
    fun showScreenshotOnboarding_whenBackFromRecentApp() {
        ...
        session {
            loadPageFromHomeSearchField(activityTestRule.activity, ajit.browser.focus.activity.ScreenshotOnBoardingTest.Companion.TARGET_URL_SITE)
