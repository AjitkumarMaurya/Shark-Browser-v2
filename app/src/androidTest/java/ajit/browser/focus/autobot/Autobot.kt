...
import ajit.browser.focus.helper.ActivityRecreateLeakWatcherIdlingResource
import ajit.browser.focus.widget.TelemetrySwitchPreference
import ajit.browser.focus.widget.TurboSwitchPreference
...
class SettingRobot {
    ...
    @Rule
    @JvmField
    val settingsActivity = ActivityTestRule(ajit.browser.focus.activity.SettingsActivity::class.java, false, false)

    private lateinit var interaction: DataInteraction

    private lateinit var leakWatchIdlingResource: ajit.browser.focus.helper.ActivityRecreateLeakWatcherIdlingResource
    ...
    fun prefSendUsageData(): SettingRobot {
        ...
        interaction = Espresso.onData(
                Is.`is`(CoreMatchers.instanceOf(ajit.browser.focus.widget.TelemetrySwitchPreference::class.java))).onChildView(ViewMatchers.withClassName(Is.`is`(Switch::class.java.name)))
        return this
    }
    ...
    fun prefTurboMode(): SettingRobot {
        // Click on the switch multiple times...
        interaction = Espresso.onData(
                Is.`is`(CoreMatchers.instanceOf(ajit.browser.focus.widget.TurboSwitchPreference::class.java))).onChildView(ViewMatchers.withClassName(Is.`is`(Switch::class.java.name)))
        return this
    }
    ...
    fun restartAndcheckNoLeak(): SettingRobot {
        ...
        leakWatchIdlingResource = ajit.browser.focus.helper.ActivityRecreateLeakWatcherIdlingResource(settingsActivity.activity)
