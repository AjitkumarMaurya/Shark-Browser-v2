...
class BrowserFragmentScreenshot : BaseScreenshot() {
        ...
        override fun beforeActivityLaunched() {
            ...
            try {
                webServer.enqueue(MockResponse()
                        .setBody(ajit.browser.focus.utils.AndroidTestUtils.readTestAsset(HTML_FILE_FULL_SCREEN_IMAGE))
                        .addHeader("Set-Cookie", "sphere=battery; Expires=Wed, 21 Oct 2035 07:28:00 GMT;"))
                webServer.enqueue(MockResponse()
                        .setBody(ajit.browser.focus.utils.AndroidTestUtils.readTestAsset(IMAGE_FILE_NAME_DOWNLOADED)))
                webServer.enqueue(MockResponse()
                        .setBody(ajit.browser.focus.utils.AndroidTestUtils.readTestAsset(IMAGE_FILE_NAME_DOWNLOADED)))
    ...
    fun screenshotBrowserFragment() {
        ...
        session {
            ...
            takeScreenshotViaFastlane(ajit.browser.focus.screengrab.ScreenshotNamingUtils.BROWSER_CONTEXT_MENU)
            clickOpenLinkInNewTab()
            checkNewTabOpenedSnackbarIsDisplayed()
            takeScreenshotViaFastlane(ajit.browser.focus.screengrab.ScreenshotNamingUtils.BROWSER_NEW_TAB_OPENED)
            loadErrorPage(TARGET_URL_ERROR_PAGE)
            takeScreenshotViaFastlane(ajit.browser.focus.screengrab.ScreenshotNamingUtils.BROWSER_ERROR_PAGE)

            // Simulate show no storage permission snackbar
            ajit.browser.focus.screengrab.MockUIUtils.showSnackbarAndWait(activityTestRule.activity, R.string.permission_toast_location, R.string.permission_handler_permission_dialog_setting)
            checkNoLocationPermissionSnackbarIsDisplayed()
            takeScreenshotViaFastlane(ajit.browser.focus.screengrab.ScreenshotNamingUtils.BROWSER_NO_LOCATION_SNACKBAR)

            // Simulate show no geo permission dialog
            ajit.browser.focus.screengrab.MockUIUtils.showGeoPromptDialog(activityTestRule.activity, TARGET_URL_ERROR_PAGE)
            checkGeoPermissionDialogIsDisplayed()

            takeScreenshotViaFastlane(ajit.browser.focus.screengrab.ScreenshotNamingUtils.BROWSER_NO_LOCATION_DIALOG)
    ...
    fun screenshotTabTray() {
        session {
            loadPageFromHomeSearchField(activityTestRule.activity, webServer.url(TEST_PATH).toString())
            clickTabTray()
            takeScreenshotViaFastlane(ajit.browser.focus.screengrab.ScreenshotNamingUtils.BROWSER_TAB_TRAY)
            clickCloseAllTabs()
            takeScreenshotViaFastlane(ajit.browser.focus.screengrab.ScreenshotNamingUtils.BROWSER_TAB_TRAY_CLOSE_DIALOG)
        }
    }
...
//            checkSearchInRocketIsDisplayed()
            takeScreenshotViaFastlane(ajit.browser.focus.screengrab.ScreenshotNamingUtils.BROWSER_TEXT_ACTION_DAILOG)
        }
    }
