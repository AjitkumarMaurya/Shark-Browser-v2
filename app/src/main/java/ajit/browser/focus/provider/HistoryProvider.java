...
import ajit.browser.focus.provider.HistoryDatabaseHelper.Tables;
import ajit.browser.focus.utils.ProviderUtils;
import ajit.browser.lightning.persistance.History.HistoryDatabase;
import ajit.browser.lightning.persistance.History.HistoryDatabase;
...
public class HistoryProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (sUriMatcher.match(uri)) {
            case BROWSING_HISTORY:
                count = db.delete(HistoryDatabaseHelper.Tables.BROWSING_HISTORY, selection, selectionArgs);
                break;
            default:
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection,
        ...
        SupportSQLiteQuery query = SupportSQLiteQueryBuilder.builder(HistoryDatabaseHelper.Tables.BROWSING_HISTORY)
                .columns(projection)
    ...
    public int update(Uri uri, ContentValues values, String selection,
        ...
        final SupportSQLiteDatabase db = mDbHelper.getWritableDatabase();
        final int count = db.update(HistoryDatabaseHelper.Tables.BROWSING_HISTORY, OnConflictStrategy.ROLLBACK, values, selection, selectionArgs);
        ...
        try {
            ...
            if (c != null) {
                if (c.moveToFirst()) {
                    ...
                    if (db.update(HistoryDatabaseHelper.Tables.BROWSING_HISTORY, OnConflictStrategy.ROLLBACK, values, BrowsingHistory._ID + " = ?", new String[]{Long.toString(id)}) == 0) {
                        id = -1;
                    }
                } else {
                    values.put(BrowsingHistory.VIEW_COUNT, 1);
                    id = db.insert(HistoryDatabaseHelper.Tables.BROWSING_HISTORY, OnConflictStrategy.ROLLBACK, values);
                }
            }
            return id;
        } finally {
