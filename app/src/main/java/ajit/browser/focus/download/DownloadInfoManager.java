...
package ajit.browser.focus.download;
...
import ajit.browser.focus.components.RelocateService;
import ajit.browser.focus.provider.DownloadContract;
import ajit.browser.lightning.util.LoggerWrapper;
import ajit.browser.threadutils.ThreadUtils;
import ajit.browser.lightning.R;
import ajit.browser.focus.components.RelocateService;
import ajit.browser.focus.utils.CursorUtils;
...
public class DownloadInfoManager {
    ...
    public void delete(Long rowId, AsyncDeleteListener listener) {
        mQueryHandler.startDelete(TOKEN, new AsyncDeleteWrapper(rowId, listener)
                , DownloadContract.Download.CONTENT_URI, DownloadContract.Download._ID + " = ?", new String[]{Long.toString(rowId)});
    }

    public void updateByDownloadId(DownloadInfo downloadInfo, AsyncUpdateListener listener) {
        mQueryHandler.startUpdate(TOKEN, listener, DownloadContract.Download.CONTENT_URI, getContentValuesFromDownloadInfo(downloadInfo)
                , DownloadContract.Download.DOWNLOAD_ID + " = ?", new String[]{Long.toString(downloadInfo.getDownloadId())});
    }

    public void updateByRowId(DownloadInfo downloadInfo, AsyncUpdateListener listener) {
        mQueryHandler.startUpdate(TOKEN, listener, DownloadContract.Download.CONTENT_URI, getContentValuesFromDownloadInfo(downloadInfo)
                , DownloadContract.Download._ID + " = ?", new String[]{Long.toString(downloadInfo.getRowId())});
    }

    public void query(int offset, int limit, AsyncQueryListener listener) {
        final String uri = DownloadContract.Download.CONTENT_URI.toString() + "?offset=" + offset + "&limit=" + limit;
    ...
    public void queryByDownloadId(Long downloadId, AsyncQueryListener listener) {
        ...
        mQueryHandler.startQuery(TOKEN, listener, Uri.parse(uri), null, DownloadContract.Download.DOWNLOAD_ID + "==?", new String[]{Long.toString(downloadId)}, null);
    }

    public void queryByRowId(Long rowId, AsyncQueryListener listener) {
        String uri = DownloadContract.Download.CONTENT_URI.toString();
        mQueryHandler.startQuery(TOKEN, listener, Uri.parse(uri), null, DownloadContract.Download._ID + "==?", new String[]{Long.toString(rowId)}, null);
    }

    public void queryDownloadingAndUnreadIds(AsyncQueryListener listener) {
        final String uri = DownloadContract.Download.CONTENT_URI.toString();
        mQueryHandler.startQuery(TOKEN, listener, Uri.parse(uri), null, DownloadContract.Download.STATUS + "!=? or " + DownloadContract.Download.IS_READ + "=?", new String[]{String.valueOf(DownloadManager.STATUS_SUCCESSFUL), String.valueOf("0")}, null);
    }
    ...
    public void markAllItemsAreRead(AsyncUpdateListener listener) {
        ...
        mQueryHandler.startUpdate(TOKEN, listener, DownloadContract.Download.CONTENT_URI, contentValues, DownloadContract.Download.STATUS + "=? and " + DownloadContract.Download.IS_READ + " = ?", new String[]{String.valueOf(DownloadManager.STATUS_SUCCESSFUL), String.valueOf("0")});
    }

    public boolean recordExists(long downloadId) {
        final ContentResolver resolver = mContext.getContentResolver();
        final Uri uri = DownloadContract.Download.CONTENT_URI;
        final String selection = DownloadContract.Download.DOWNLOAD_ID + "=" + downloadId;
    ...
    private static ContentValues getContentValuesFromDownloadInfo(DownloadInfo downloadInfo) {
        final ContentValues contentValues = new ContentValues();
        contentValues.put(DownloadContract.Download.DOWNLOAD_ID, downloadInfo.getDownloadId());
        contentValues.put(DownloadContract.Download.FILE_PATH, downloadInfo.getFileUri());
        contentValues.put(DownloadContract.Download.STATUS, downloadInfo.getStatus());
    ...
    private static DownloadInfo cursorToDownloadInfo(Cursor cursor) {
        final long downloadId = cursor.getLong(cursor.getColumnIndex(DownloadContract.Download.DOWNLOAD_ID));
        String fileUri = cursor.getString(cursor.getColumnIndex(DownloadContract.Download.FILE_PATH));
        Long rowId = cursor.getLong(cursor.getColumnIndex(DownloadContract.Download._ID));
