...
import ajit.browser.focus.firstrun.DefaultFirstrunPagerAdapter
import ajit.browser.focus.firstrun.UpgradeFirstrunPagerAdapter
...
import ajit.browser.lightning.periodic.PeriodicReceiver
...
class FirstrunFragment : Fragment(), View.OnClickListener, Screen {
    ...
    override fun onClick(view: View) {
        when (view.id) {
            ...
            R.id.finish -> {
                ...
                if (isTelemetryValid) {
                    val mode = ajit.browser.focus.utils.NewFeatureNotice.getInstance(context).lastShownFeatureVersion
    ...
    private fun promoteSetDefaultBrowserIfPreload(): FirstrunFragment {
        ...
        if (isSystemApp()) {
            ajit.browser.focus.utils.DialogUtils.showDefaultSettingNotification(context)
        }
        return this
    }
    ...
    private fun findPagerAdapter(context: Context, onClickListener: View.OnClickListener): PagerAdapter? {
        ...
        val shown = ajit.browser.focus.utils.NewFeatureNotice.getInstance(getContext()).hasShownFirstRun()
        pagerAdapter = if (!shown) {
            ajit.browser.focus.firstrun.DefaultFirstrunPagerAdapter(context, wrapButtonClickListener(onClickListener))
        } else if (ajit.browser.focus.utils.NewFeatureNotice.getInstance(getContext()).shouldShowLiteUpdate()) {
            ajit.browser.focus.firstrun.UpgradeFirstrunPagerAdapter(context, onClickListener)
        } else {
            null
        }
        return pagerAdapter
    ...
    private fun finishFirstrun() {
        ajit.browser.focus.utils.NewFeatureNotice.getInstance(context).setFirstRunDidShow()
        ajit.browser.focus.utils.NewFeatureNotice.getInstance(context).setLiteUpdateDidShow()

        (activity as ajit.browser.focus.activity.MainActivity).firstrunFinished()
    }
