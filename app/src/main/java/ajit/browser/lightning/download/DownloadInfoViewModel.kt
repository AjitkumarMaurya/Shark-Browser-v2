...
class DownloadInfoViewModel(private val repository: DownloadInfoRepository) : ViewModel() {
    ...
    val toastMessageObservable = ajit.browser.lightning.download.SingleLiveEvent<Int>()
    val deleteSnackbarObservable = ajit.browser.lightning.download.SingleLiveEvent<ajit.browser.focus.download.DownloadInfo>()
    var isOpening = false
    ...
    private var updateListener: DownloadInfoRepository.OnQueryItemCompleteListener = object : DownloadInfoRepository.OnQueryItemCompleteListener {
        override fun onComplete(download: ajit.browser.focus.download.DownloadInfo) {
            updateItem(download)
        }
    }
    ...
    private fun updateItem(downloadInfo: ajit.browser.focus.download.DownloadInfo) {
        var index = -1
    ...
    fun loadMore(init: Boolean) {
        ...
        repository.loadData(itemCount, PAGE_SIZE, object : DownloadInfoRepository.OnQueryListCompleteListener {
            override fun onComplete(list: List<ajit.browser.focus.download.DownloadInfo>) {
    ...
    fun confirmDelete(download: ajit.browser.focus.download.DownloadInfo) {
    ...
    fun add(downloadInfo: ajit.browser.focus.download.DownloadInfo) {
        var index = -1
        for (i in 0 until downloadInfoPack.list.size) {
            if (downloadInfoPack.list[i].rowId < downloadInfo.rowId) {
                index = i
    ...
    fun queryDownloadProgress() {
        repository.queryDownloadingItems(runningDownloadIds, object : DownloadInfoRepository.OnQueryListCompleteListener {
            override fun onComplete(list: List<ajit.browser.focus.download.DownloadInfo>) {
                if (list.isNotEmpty()) {
                    ajit.browser.threadutils.ThreadUtils.postToMainThread {
