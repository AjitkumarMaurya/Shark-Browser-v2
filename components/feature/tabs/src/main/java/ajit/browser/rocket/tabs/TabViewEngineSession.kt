...
class TabViewEngineSession constructor(
    ...
    var tabView: ajit.browser.rocket.tabs.TabView?
        set(value) {
    ...
    private var engineView: ajit.browser.rocket.tabs.TabView? = null
    ...
    interface Observer : EngineSession.Observer {
        ...
        fun onLongPress(hitTarget: ajit.browser.rocket.tabs.TabView.HitTarget)
        fun onEnterFullScreen(callback: ajit.browser.rocket.tabs.TabView.FullscreenCallback, view: View?)
    ...
    interface Client {
        ...
        fun onHttpAuthRequest(callback: ajit.browser.rocket.tabs.TabViewClient.HttpAuthCallback, host: String?, realm: String?)
    }

    class ViewClient(private val es: TabViewEngineSession) : ajit.browser.rocket.tabs.TabViewClient() {
    ...
    class ChromeClient(private val es: TabViewEngineSession) : ajit.browser.rocket.tabs.TabChromeClient() {
        ...
        override fun onCloseWindow(tabView: ajit.browser.rocket.tabs.TabView?) {
        ...
        override fun onShowFileChooser(
                tabView: ajit.browser.rocket.tabs.TabView,
                filePathCallback: ValueCallback<Array<Uri>>?,
                fileChooserParams: WebChromeClient.FileChooserParams?
        ): Boolean {
        ...
        override fun onReceivedTitle(view: ajit.browser.rocket.tabs.TabView, title: String?) {
            if (title != null) {
        ...
        override fun onReceivedIcon(view: ajit.browser.rocket.tabs.TabView, icon: Bitmap?) =
                es.notifyObservers { onReceivedIcon(icon) }

        override fun onLongPress(hitTarget: ajit.browser.rocket.tabs.TabView.HitTarget) =
                es.notifyObservers { onLongPress(hitTarget) }

        override fun onEnterFullScreen(callback: ajit.browser.rocket.tabs.TabView.FullscreenCallback, view: View?) =
    ...
    class FindListener(private val es: TabViewEngineSession) : ajit.browser.rocket.tabs.TabView.FindListener {
        override fun onFindResultReceived(activeMatchOrdinal: Int, numberOfMatches: Int, isDoneCounting: Boolean) {
            es.notifyObservers { onFindResult(activeMatchOrdinal, numberOfMatches, isDoneCounting) }
        }
    }

    class DownloadCallback(private val es: TabViewEngineSession) : ajit.browser.rocket.tabs.web.DownloadCallback {
